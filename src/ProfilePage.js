...
class ProfilePage extends Component {
  ...
  constructor (props) {
    ...
    this.props.api.loadProfile(props.id, props.state, p => this.setState({profile: p}));
  ...
  toggleProfileState () {
    ...
    this.props.api.changeProfileState(this.props.id, oldState, newState, data => {

      this.props.history.push(`/${this.props.id}/${newState}`);
    })
  }

  eraseProfile () {

    this.props.api.eraseProfile(this.props.id, this.props.state, data => {
  ...
  render() {
    ...
    function codeToMissionName (code) {
          ...
          switch (code) {
                ...
                case 2: return "DPO interne"
                case 3: return "DPO externe"
                case 4: return "Audit"
                case 5: return "Assistance juridique"
                default: return "?"
          }
    }
          ...
          <main>
            <div className="profile-container">
              <div className="profile-header">
                <img className="avatar" src={this.state.profile.data.avatar} alt="avatar"/>
                <h3>{this.state.profile.data.fullname}</h3>
                {
                  this.props.state === "published" ?
                    <Badge color="secondary">Publié</Badge> :
                    <Badge color="warning">En attente</Badge>
                }
              </div>
              <div className="half-profile">
                <div className="profile-section">
                  <h2>Offres</h2>
                  <div className="badge-list">
                    { this.state.profile.data.offers.map(o => <Badge key={o} color="primary">{codeToMissionName(o)}</Badge>) }
                  </div>
                </div>
                <div className="profile-section">
                  <h2>Secteurs d'activité</h2>
                  <div className="tag-list">
                    { this.state.profile.data.tags.map(tag => <div key={tag} className="tag">{tag}</div>) }
                  </div>
                </div>
                <div className="profile-section">
                  <h2>Zones d'intervention</h2>
                  <ListGroup>
                        { this.state.profile.data.areas.map(tag => <ListGroupItem key={"area-" + tag}>{codes[tag]}</ListGroupItem>) }
                  </ListGroup>
                </div>
                <h2>Informations de contact</h2>
                {
                  this.state.profile.data.publicEmail === undefined && this.state.profile.data.tel === undefined && this.state.profile.data.website === undefined && this.state.profile.data.company === undefined ?
                      <div className="no-info-text">Aucune information spécifiée</div> :
                      <div>
                      {
                            this.state.profile.data.publicEmail !== undefined ?
                                  <div className="contact-info">
                                        ...
                                        <span className="contact-text">
                                              {this.state.profile.data.publicEmail}
                                        </span>
                                  </div> :
                                  <span/>
                      }
                      {
                            this.state.profile.data.tel !== undefined ?
                                  <div className="contact-info">
                                        ...
                                        <span className="contact-text">
                                              {this.state.profile.data.tel}
                                        </span>
                                  </div> :
                                  <span/>
                      } 
                      {
                            this.state.profile.data.website !== undefined ?
                                  <div className="contact-info">
                                        ...
                                        <span className="contact-text">
                                              <a href={this.state.profile.data.website} target="_blank">{this.state.profile.data.website}</a>
                                        </span>
                                  </div> :
                                  <span/>
                      } 
                      {
                            this.state.profile.data.company !== undefined ?
                                  <div className="contact-info">
                                        ...
                                        <span className="contact-text">
                                              {this.state.profile.data.company}
                                        </span>
                                  </div> :
                                  <span/>
                      }
              ...
              </div><div className="half-profile">
                <div className="simple-container">
                  <div className="bio-content" dangerouslySetInnerHTML={{ __html: this.state.profile.data.resume}}/>
                </div>
              </div>
            </div>
            <div className="choicer">
              { this.props.state === "published" ? <Button outline color="warning" onClick={() => this.toggleProfileState()}>Mettre en attente</Button> : <Button outline color="secondary" onClick={() => this.toggleProfileState()}>Publier</Button> }
              <Button outline color="danger" onClick={() => this.eraseProfile()}>Supprimer</Button>
            </div>
          </main>
        }
        <footer/>
